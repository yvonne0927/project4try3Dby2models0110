...
function initThreeOverlay() {
  ...
  three.camera.position.set(0, 1.2, 2.2);   // ✅ 相机抬高到 y>0
  three.camera.lookAt(0, 0, 0);             // ✅ 看向地面原点（y=0）
...
// F) Apply model + material + position (A & B) —— 完全重叠在 spawn 点
if (three.ready && objects.length > 0) {

  // 1) 统一 spawn 点（优先用 spawn2D；没有就用扫描框中心）
  const spawnX = (spawn2D && spawn2D.ok) ? spawn2D.x : (bx + boxSize / 2);
  const spawnY = (spawn2D && spawn2D.ok) ? spawn2D.y : (by + boxSize / 2);

  // 2) 只算一次地面交点
  const hit = screenToGround(spawnX, spawnY);

  // === A ===
  const objA = objects[0];
  const instA = ensureInstanceForSlot("A", objA.organism);
  if (instA) {
    const appA = makeAppearance(objA.feats, objA.organism);
    applyAppearanceToModel(instA, appA);

    if (hit) {
      instA.visible = true;
      instA.position.set(hit.x, hit.y + 0.05, hit.z); // ✅ 不偏移
      // 可选：避免每帧累加导致转太快
      instA.rotation.y = 0;
    }
  }
  ...
  if (objects.length > 1) {
    ...
    const instB = ensureInstanceForSlot("B", objB.organism);
    if (instB) {
      const appB = makeAppearance(objB.feats, objB.organism);
      applyAppearanceToModel(instB, appB);

      if (hit) {
        instB.visible = true;
        instB.position.set(hit.x, hit.y + 0.05, hit.z); // ✅ 不偏移（完全重叠）
        instB.rotation.y = 0;
      }
    }
  } else {
